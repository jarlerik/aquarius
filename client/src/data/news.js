export default {
	fi: {
		header: "Tie hyvään mieleen ja kuntoon",

		p1: `Meillä voit kuntoilla valoisissa tiloissa, joista avautuu upea
			merinäköala. Käytettävissäsi ovat Technogymin huippumodernit
			laitteet, mm. Selection Line sekä Kinesis, missä voit treenata
			lihasvoimaa, tasapainoa sekä liikkuvuutta – kaikkea yhtä aikaa!
			Olet myös tervetullut kokeilemaan erilaisia Les Millsin tunteja
			sekä meidän muita jumppia.`,

		p2: `Kuntoilun jälkeen voit rentoutua viihtyisässä saunassa.
			Hyvä treeni virkistää sekä mielesi, että kehosi ja olet täynnä
			energiaa!`
	},
	sv: {
		header: `Vägen till god hälsa går via Aquarius!`,

		p1: `Hos oss kan du träna i ljusa och rymliga utrymmen med en
			fantastisk havsutsikt! Vi har toppmodern utrustning från
			Technogym, bl.a. Selection Line och Kinesis där du kan träna
			muskelstyrka, balans och vighet - allt i ett!
			Du är även välkommen och pröva olika LesMills timmar och
			våra andra jumppor.`,

		p2: `Efter träningen kan du koppla av i våra trivsamma bastun. En
			god träning uppfriskar kropp och själ och du är full av energi!`
	}
}